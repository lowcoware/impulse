...
This file carries the impulse-core ruleset for Google Antigravity (IDE, Antigravity 2.0 desktop, and CLI), delivered as a plain-markdown rule file per `antigravity.google/docs/rules-workflows/`. It ships with zero frontmatter because the exact YAML key for the "Always On" activation mode is not confirmed by research as of this writing — after install, open Antigravity's Rules UI/picker for this workspace and set this file's activation mode to **Always On** yourself; Manual, Model Decision, and Glob are the other three modes and none of them give the always-active behavior this ruleset assumes.
