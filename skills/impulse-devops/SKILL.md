...
| references/ci.md | build/test/lint gate, SSH auto-deploy + server-access model comparison, health gate, `pull_request_target` exfil, concurrency groups, SHA-pinned actions, caching stack, 10-min PR budget, runner economics + minute-billing optimization (runner-size cost ratio, monorepo affected-only CI, draft-PR gating, self-hosted ROI, native spend budgets), security-scan stack (trivy/trufflehog/hadolint/lychee), Semgrep diff-aware SAST gate with SARIF + nosemgrep discipline — IDs `DO-CI1`–`DO-CI21` | GitHub Actions workflow, cutting Actions minute spend |
