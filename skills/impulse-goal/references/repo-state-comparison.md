...
A two-dot range compares two **commits**. An autonomous Impulse-goal run frequently leaves its
...
namespaced artifact dir, e.g. `.impulse-goal/add-dark-mode-Ab3Kx9`), so the `/goal` session invokes
...
The transcript markers (`IMPULSEGOAL_PHASE_VERIFY` cleanliness section, `AUDIT_VERIFY`
