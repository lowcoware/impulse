...
## Impulse-goal's single-`/goal` shape

Impulse-goal uses **one** `/goal` per run, dispatched by the **user** at the end of Stage 7. Slash commands fire only from user input on both Claude Code and Codex — the planner cannot fire `/goal` from its own message text. Stage 7's job is to write all phase specs to disk, then print a copy-paste-ready `/goal` block. The user pastes once; from there, the run is autonomous.

The condition is (`<run-root>` is this run's namespaced artifact dir, e.g. `.impulse-goal/add-dark-mode-Ab3Kx9`; the planner substitutes the concrete path before printing the block so the pasted line carries the real directory — never the placeholder):

```
Execute all phases of <run-root>/ROADMAP.md sequentially.
Read <run-root>/phases/phase-N.md for each phase; do the work;
run mandatory commands; print IMPULSEGOAL_PHASE_VERIFY then
IMPULSEGOAL_PHASE_DONE for each phase; follow the failure-recovery
...
Done when IMPULSEGOAL_RUN_COMPLETE appears in the transcript with
one IMPULSEGOAL_PHASE_DONE per phase, AUDIT_COMPLETE printed before
IMPULSEGOAL_RUN_COMPLETE, and no FAILURE_HANDOFF or AUDIT_HANDOFF
this run.
```
...
## Required transcript blocks (Impulse-goal-specific)
...
### `IMPULSEGOAL_PHASE_START` (once per phase, at execution start)
...
### `IMPULSEGOAL_PHASE_VERIFY` (once per phase, before DONE)
...
### `IMPULSEGOAL_PHASE_DONE` (once per phase, final block of the phase)
...
### `AUDIT_COMPLETE` (zero gaps — emit before IMPULSEGOAL_RUN_COMPLETE)
...
### `IMPULSEGOAL_RUN_COMPLETE` (once, after AUDIT_COMPLETE)
...
- **Don't make conditions the evaluator can't verify from the transcript.** "Tests pass" is wrong (evaluator can't run tests); "`IMPULSEGOAL_PHASE_DONE` printed for all phases" is right.
