#!/usr/bin/env bash
# claim-run.sh — atomically claim a unique per-run artifact directory under .impulse-goal/.
#
# Why this exists
# ---------------
# Two `/impulse-goal` runs started in the same working tree both defaulted to a flat
# `.impulse-goal/` directory and clobbered each other's STATE.md / ROADMAP.md / phases/.
...
#                                          e.g.  .impulse-goal/add-dark-mode-Ab3Kx9
#
# Nothing but the path is printed, so callers can capture it directly:
#   IMPULSEGOAL_ROOT="$(bash claim-run.sh "$ARGUMENTS")"
#
# Honours $IMPULSEGOAL_BASE (default ".impulse-goal") as the parent directory.
