# Impulse-goal execution protocol
...
Repeat until `IMPULSEGOAL_RUN_COMPLETE` is printed:

1. Read `{{RUN_ROOT}}/STATE.md`. Find `Current phase: N`.
2. Read `{{RUN_ROOT}}/phases/phase-N.md`. This is your full work spec.
3. Print `IMPULSEGOAL_PHASE_START` with the spec's metadata (phase number, name, task, mandatory commands, acceptance count, evidence types, dependencies).
...
7. Print `IMPULSEGOAL_PHASE_DONE`. Update `STATE.md`: mark phase N completed; set `Current phase: N+1`; bump `Last update` timestamp; append a one-line event.
...
10. If N == total: do **not** print `IMPULSEGOAL_RUN_COMPLETE` yet. Run the **Final audit** below. Only after `AUDIT_COMPLETE`, print `IMPULSEGOAL_RUN_COMPLETE` with a 5-line summary. The `/goal` condition is satisfied at that point.
...
3. **Phase completeness:** scan the transcript for one `IMPULSEGOAL_PHASE_DONE` per phase 1..N. Any missing = an `AUDIT_GAP`.
...
5. On 3rd round's audit failure: print `AUDIT_HANDOFF` (full gap history, suggested next move), update `STATE.md` to `BLOCKED`, stop. Do **not** print `IMPULSEGOAL_RUN_COMPLETE`.
...
3. Print `IMPULSEGOAL_RUN_COMPLETE` with the 5-line summary. If `trust_prior / (re_verified + trust_prior)` > **30%**, prepend a one-line honesty banner: `⚠ Audit coverage: <re_verified> re-verified, <trust_prior> trust-prior (<pct>%). Eyeball UI/UX before merging.` Below 30%, print the same coverage line without the warning prefix.
...
- Pause at the next phase boundary (after `IMPULSEGOAL_PHASE_DONE`, before reading the next spec).
- Address the message.
...
- Save anything non-obvious a future Impulse-goal run on a similar task would benefit from.
...
- `IMPULSEGOAL_PHASE_VERIFY`
- `MEMORY_SAVED`
- `IMPULSEGOAL_PHASE_DONE`
