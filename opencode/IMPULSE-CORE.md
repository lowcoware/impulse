# impulse-core for OpenCode

This file carries the impulse-core always-on ruleset in plain markdown, meant to be delivered to OpenCode through its static instructions system — either referenced by path (or URL) from the `instructions` array in a project's or the user's global `opencode.json`, or copied wholesale into the user's own project or global `AGENTS.md`. OpenCode has no working per-turn injection hook yet, so this file is the delivery vehicle, not a plugin.
